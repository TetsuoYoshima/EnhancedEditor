...
namespace EnhancedEditor.Editor {
    ...
    /// </summary>
	public sealed class PinboardWindow : EditorWindow {
        #region Indent Level
        ...
        private sealed class TypeComparer : PinObjectComparer {
        ...
        private sealed class NameComparer : PinObjectComparer {
        ...
        private sealed class PathComparer : PinObjectComparer {
            ...
            if (_event.type == EventType.DragPerform) {
                ...
                if (DragAndDrop.GetGenericData(PinObjectDataType) is PinObject[] _pinObjects) {
                    ...
                    for (int _i = 0; _i < _pinObjects.Length; _i++) {
                        ...
                        while (_indentHelper.SafeLast(out int _lastIndent) && (_objectIndent <= _lastIndent)) {
                        ...
                        if (_pinObject.IsFolder && _pinObject.IsSelected) {
                            ...
                            if (_indexOf >= _index)
                                _index++;
                        } else if (_indentHelper.SafeLast(out int _lastIndent) && !_pinObject.IsSelected) {
